...
describe("did:key method driver", () => {
  describe("get", () => {
    it("should get the DID Document for a did:key DID", async () => {
      ...
      expect(publicKey.publicKeyBase58).to.equal("B12NYF8RrR3h41TDCTJojY59usg3mbtbjnFs7Eud1Y6u");

      const [kak] = didDoc.keyAgreement;
      expect(kak.id).to.equal(did + "#z6LSbysY2xFMRpGMhb7tFTLMpeuPRaqaWM1yECx2AtzE3KCc");
      expect(kak.type).to.equal("X25519KeyAgreementKey2019");
      expect(kak.controller).to.equal(did);
      expect(kak.publicKeyBase58).to.equal("JhNWeSVLMYccCk7iopQW4guaSJTojqpMEELgSLhKwRr");
    });
    ...
    it("should resolve an individual key within the DID Doc", async () => {
      ...
      const key = await driver().get(keyId);
    ...
    it("should resolve an individual key agreement key", async () => {
      ...
      const key = await driver().get(kakKeyId);

      expect(key).to.eql({
        "@context": "https://w3id.org/security/v2",
        // eslint-disable-next-line max-len
        id:
          "did:key:z6MkpTHR8VNsBxYAAWHut2Geadd9jSwuBV8xRoAnwWsdvktH#z6LSbysY2xFMRpGMhb7tFTLMpeuPRaqaWM1yECx2AtzE3KCc",
  ...
  describe("computeKeyId", async () => {
    ...
    it("should set the key id based on fingerprint", async () => {
      const keyId = await driver().computeKeyId({ key });

      expect(keyId).to.equal("did:key:12345#12345");
    });
  });

  describe("method", () => {
    it("should return did method id", async () => {
      expect(driver().method).to.equal("key");
    });
  });
});

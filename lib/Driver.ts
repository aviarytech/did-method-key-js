...
class DidDocument {
  ...
  keyAgreement: KeyBlock[];
  keys?: any[];

  static async fromKey(edKey) {
...
export class Driver {
  ...
  public _cache: LRUCache<string, any>;

  constructor({ maxCacheSize = 100 } = {}) {
    // used by did-io to register drivers
    this.method = "key";
    this._cache = new LRUCache({ max: maxCacheSize });
  }
   ...
   */
  async get(did: string, url?: string) {
    did = did || url;
    if (!did) {
   ...
   */
  async generate({ keyType = "Ed25519VerificationKey2018" } = {}): Promise<DidDocument> {
    const publicKey = await LDKeyPair.generate({ type: keyType });
    return DidDocument.fromKey(publicKey);
  }
   ...
   */
  async computeKeyId({ key }): Promise<string> {
